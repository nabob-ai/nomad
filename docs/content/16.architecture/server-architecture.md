...
nomad Server 是一个生产级的 HTTP 服务器实现，提供完整的认证、授权、可观测性和速率限制功能。
...
```go
metrics := observability.NewMetricsManager("nomad")

// HTTP 指标
nomad_http_requests_total{method,path,status}
nomad_http_request_duration_seconds{method,path}
...
```bash
nomad serve --port 8080 --mode debug
```
