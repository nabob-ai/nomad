...
- 使用 `nomad eval` CLI 在命令行中对文本进行评估。
...
当你使用 `nomad serve` 启动 HTTP Server 后,会自动暴露一个本地评估接口:
...
除了 HTTP API,你还可以直接在命令行使用 `nomad eval` 进行本地评估,非常适合:
...
- `-json`
  ...
  ```bash
  echo "text..." | nomad eval -reference "ref..." -json
  ```
...
- `-file path`
  ...
  ```bash
  nomad eval -file cases.jsonl -json
  ```
...
除了基于启发式算法的 Scorer，nomad 还提供了 8 个基于 LLM 的 Scorer，用于更高级的评估任务。这些 Scorer 使用 LLM 作为 judge 来评估文本质量。
