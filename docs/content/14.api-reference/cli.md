---
title: nomad CLI 示例
description: 使用 nomad serve 启动标准 HTTP Chat 服务
navigation: false
---

# nomad CLI 示例

nomad 提供了一个简单的 CLI 可执行程序 `nomad`, 用于快速启动一个标准化的 HTTP Chat 服务(当前为最小可用版本)。

> 示例代码路径: `cmd/nomad/main.go`
...
serve Flags:
  ...
  -store string
        Directory for JSON store data (default ".nomad")
  -template string
...
一个最小可用的 `../../../namad.yaml` 示例(已包含在仓库根目录):
