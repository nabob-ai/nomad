...
nomad Server 提供完整的 Prometheus metrics 支持，用于监控应用性能和业务指标。
...
```
nomad_http_requests_total{method="GET",path="/v1/agents",status="2xx"}
```
...
```
nomad_http_request_duration_seconds{method="GET",path="/v1/agents"}
```
...
```
nomad_http_request_size_bytes{method="POST",path="/v1/agents"}
```
...
```
nomad_http_response_size_bytes{method="GET",path="/v1/agents"}
```
...
```promql
rate(nomad_http_requests_total[5m])
```
...
histogram_quantile(0.99,
  rate(nomad_http_request_duration_seconds_bucket[5m])
)
```
...
```promql
rate(nomad_http_requests_total{status="5xx"}[5m])
  /
rate(nomad_http_requests_total[5m])
```
...
```promql
rate(nomad_http_response_size_bytes_sum[5m])
  /
rate(nomad_http_response_size_bytes_count[5m])
```
