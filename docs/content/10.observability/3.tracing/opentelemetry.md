...
nomad Server 内置 OpenTelemetry 支持，提供完整的分布式追踪能力。
...
export OTEL_SERVICE_NAME=nomad
...
rate(http_request_duration_seconds_bucket{job="nomad"}[5m])
```
...
3. **检查日志**
   ```bash
   # 查看 exporter 错误
   docker logs nomad 2>&1 | grep "trace"
   ```
