...
description: 使用 nomad 事件系统和 SSE 构建一个类似 Claude Code 的分阶段工具调用视图
---
...
func main() {
    ...
    storePath := ".nomad-plan-explore-ui"
...
1. 规划(Plan): 分析 nomad 仓库里 pkg/tools/builtin 目录下各个工具的职责和测试需求, 给出一个分步骤的实施计划。
...
本示例使用的是「单 Agent + TodoList 中间件」模式。如果结合前文实现的 `Task` + `nomad subagent` 子代理框架:
