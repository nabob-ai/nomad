...
description: 使用可插拔向量存储 + Embedder 为 nomad 增强 RAG 能力
---

# 语义记忆(Semantic Memory) 示例

nomad 默认采用 **文件 + grep** 的方式实现长期记忆, 对很多场景(用户/项目/资源级记忆)已经足够简单可靠。但对于部分用户, 仍然希望接入向量数据库/语义检索(RAG)。
...
下面的组件可以直接调用 `nomad serve` 暴露的 `/v1/memory/semantic/search` 接口, 体验语义检索效果:
...
## 4. 通过 nomad.yaml 启用语义记忆

在 `../../../../namad.yaml` 中可以声明向量存储、embedder 和语义记忆配置:
...
如果你已经在 PostgreSQL 中安装了 pgvector 扩展, 并希望使用 OpenAI 的 embedding 模型, 可以在 `../../../../namad.yaml` 中这样配置:

```yaml
vector_stores:
  - name: main
    kind: pgvector
    dsn: postgres://user:password@localhost:5432/nomad?sslmode=disable
    table: agent_vectors
    metric: cosine
    dimension: 1536
...
- 如果你需要生产级 RAG, 建议:
  ...
  - 在 `../../../../namad.yaml` 中添加相应的 `kind` 和连接参数。
