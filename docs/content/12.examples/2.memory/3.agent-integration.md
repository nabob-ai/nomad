...
- Agent 可以使用：
  ...
  - `Read` / `Write` / `Grep` 等工具查看和操作记忆文件。
- `/agent.md` 的内容会被自动注入到 System Prompt，作为基础“人格/长期指令”。

> 示例代码路径：`examples/memory-agent/main.go`

如果你希望在此基础上引入「语义长期记忆」, 可以在应用层再接入：

- `SemanticMemory` + `VectorStore`/`Embedder`（见 `examples/memory-semantic`）；
- `LongTermBridge`（见 `examples/memory-semantic-session`）用于将特定 Session 的知识性内容保存到语义索引中。
...
func main() {
    ...
    jsonStore, _ := store.NewJSONStore(".nomad-memory-agent")
