---
title: JS 客户端示例
description: 使用 @nomad/client-js 访问 HTTP Chat API
---

# JS 客户端示例

为方便前端/Node.js 应用集成, nomad 提供了一个最小的 JS/TS 客户端包:

- 包路径: `client-sdks/client-js`
- NPM 名称: `@nomad/client-js`
...
const client = new AgentsdkClient({
  baseUrl: "http://localhost:8080", // nomad serve 的地址
});
...
- `docs/public/openapi/nomad.yaml`
...
- `nomad serve` 提供的 HTTP Chat(同步/流式)接口
- `@nomad/client-js` 封装的同步 Chat 客户端
- MCP Server(`nomad mcp-serve`) 暴露 docs / 项目工具

你可以在自己的前端项目中构建一个简单的 Playground:

- 使用 `@nomad/client-js` 完成交互式 Chat。
