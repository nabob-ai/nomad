...
```typescript
await client.memory.semantic.store("nomad is a powerful framework for building AI agents", {
...
```typescript
const chunks = await client.memory.semantic.search("What is nomad?", {
  limit: 5,
  threshold: 0.7,
  filter: {
    category: "introduction",
